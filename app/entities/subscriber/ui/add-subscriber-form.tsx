...
import { ContactPoints, AddSubscriberModalTypeData } from "@entities/subscriber/model";
...
const AddSubscriberForm = ({ opened, onClose }: AddSubscriberModalTypeData): ReactNode => {
  const [contactPoints, setContactPoints] = useState<ContactPoints[]>([]);
  const [hasUnicode, setHasUnicode] = useState(false);
  const addSubscriber = useSubscriberState(state => state.addSubscriber);
  ...
  const handleAddContactPoints = (): void => {
    if (contactForm.validate().hasErrors) return;
    setContactPoints([
      ...contactPoints,
      {
  ...
  const handleSubmit = (e: { preventDefault: () => void }): void => {
    ...
    if (subscriberForm.isValid() && contactForm.isValid()) {
      addInfo({ userName: "iss", password: "shjhSjASDjk23u", showModal: true });
      addSubscriber({ ...subscriberForm.values, contactPoints });
      onClose();
    }
    e.preventDefault();
  };
  ...
  return (
    <Modal
      title={subscriberLanguage.management.modal.label}
      opened={opened}
      centered
      size="xl"
      onClose={onClose}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Group justify="flex-end" mt="lg">
            ...
            <Button w={100} type="submit">
              {subscriberLanguage.management.modal.creat}
            </Button>
          </Group>
        </form>
      </Box>
